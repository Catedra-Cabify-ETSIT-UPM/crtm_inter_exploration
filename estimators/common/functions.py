...
def get_running_times(cod_stop_from, cod_stop_to, arrival_times_path=None):
    """Get the running times between two stops for past trips"""
    # create sqlalchemy engine
    engine = create_engine(engine_string)
    trip_times_array = []
    
    if arrival_times_path is not None:
        arrival_times = pd.read_csv(arrival_times_path, parse_dates=['arrival_time']).query("cod_stop == '{cod_stop_from}' or cod_stop == '{cod_stop_to}'".format(cod_stop_from=cod_stop_from, cod_stop_to=cod_stop_to))
    else:
...
calendar = pd.read_csv('madrid_working_days.csv',
                       sep=';',
                       parse_dates=['Dia'],
                       usecols=[0, 2]
                       )
...
def get_metrics_anytime(plot_df):
    """Return several metrics for the DataFrame"""
    print("CRTM API MAE: " + str(np.mean(
        np.abs(
            plot_df['error']/60
        ))) + " minutes.")
    print("Model MAE: " + str(np.mean(
        np.abs(
            plot_df['error_corrected']/60
        ))) + " minutes.")
    print("CRTM API RMSE: " + str(np.sqrt(np.mean(np.square(plot_df['error']/60)))) + " minutes.")
    print("Model RMSE: " + str(np.sqrt(np.mean(np.square(plot_df['error_corrected']/60)))) + " minutes.")
    
    print(plot_df[['error', 'error_corrected']].describe())
